...
def geolookup(
    ...
    Returns
    -------
    A DataFrame representing the location with contextual information from the Census
    Bureau.
...
def batch_geocode(
    ...
    if df.shape[0] > 10000:
        raise ValueError(
            "The row limit for the Census batch geocoder is 10,000. "
            "Consider splitting your request into multiple requests and retry."
        )
    ...
    output = pd.read_csv(
        StringIO(req.text), sep=",", header=None, quoting=csv.QUOTE_ALL
    )
