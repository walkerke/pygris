...
def get_census(
    ...
    Notes
    -----
    This function is a low-level interface to the Census APIs provided for convenience.
    For a full-featured, Pythonic interface to the US Census Bureau APIs, I would
    recommend using the cenpy package (https://cenpy-devs.github.io/cenpy/index.html)
    ...
    for chunk in np.array_split(variables, n_chunks):
        ...
        if req.status_code != 200:
            raise SyntaxError(
                f"Request failed. The Census Bureau error message is {req.text}"
            )
        ...
        if return_geoid:
            ...
            # if 'state' is not in the list of columns, don't assemble the GEOID;
            # too much ambiguity among possible combinations across the various
            # endpoints
            if "state" not in my_cols:
                raise ValueError(
                    "`return_geoid` is not supported for this geography hierarchy."
                )
            ...
            df = df.drop(geoid_cols, axis=1)

        if guess_dtypes:
            num_list = []
            # Iterate through the columns in variables and try to guess if they should
            # be converted
            for v in chunk:
                check = pd.to_numeric(df[v], errors="coerce")
                # If the columns aren't fully null, convert to numeric, taking care
                # of any oddities
                if not pd.isna(check.unique())[0]:
                    df[v] = check
                    num_list.append(v)

            # If we are guessing numerics, we should convert NAs
            # (negatives below -1 million) to NaN. Users who want to keep the codes
            # should keep as object and handle themselves.
...
def get_lodes(
    ...
    """
    Get synthetic block-level data on workplace, residence, and origin-destination
    flows characteristics from the LEHD Origin-Destination Employment Statistics
    (LODES) dataset

    Parameters
    ----------
    state : str
        The state postal code of your requested data. Please note that not all states
        are available in all years.
    year : int
        The year of your requested data. LODES data go back to 2002, but not all
        datasets are available for all years / for all states.
    version : str
        The LODES version to use.  Version 8 (the default, use "LODES8") is enumerated
        at 2020 Census blocks. "LODES7" is enumerated at 2010 Census blocks, but ends
        in 2019; "LODES5" is enumerated at 2000 Census blocks, but ends in 2009.
    lodes_type : str
        One of "od" (the default) for origin-destination flows, "wac" for workplace
        area characteristics, or "rac" for residence area characteristics.
    part : str
        Only relevant for the "od" file.  "main" gives information on within-state
        residence to workplace flows. "aux" gives information for residence to
        workplace flows from outside a given state.
    job_type : str
        The available job type breakdown; defaults to "JT00" for all jobs. Please
        review the LODES technical documentation for a description of other options.
    segment : str
        The workforce segment, relevant when lodes_type is "wac" or "rac". Defaults
        to "S000" for total jobs; review the LODES technical documentation for a
        description of other options.
    agg_level : str
        The level at which to aggregate the data.  Defaults to the Census block; other
        options include "county", "tract", and "block group".
    cache : bool
        If True, downloads the requested LODES data to a cache directory on your
        computer and reads from that directory if the file exists. Defaults to False,
        which will download the data by default.
    return_geometry : bool
        If True, get_lodes() will fetch the corresponding polygon geometry for shapes
        and return a GeoPandas GeoDataFrame.  Defaults to False.
    return_lonlat : bool
        If True, columns representing the corresponding polygon centroid will be
    od_geometry : str
        Whether to attach residential geometries ("home") or workplace geometries
        ("work").  Only specified when lodes_type is "od".  Defaults to "home".
    cb : bool
        If retrieving geometry, use the Cartographic Boundary shapefile (True) or the
        TIGER/Line shapefile (False). Defaults to True for LODES8 and LODES7, and
        False for LODES5.
    ...
    Notes
    -----
    Please review the LODES technical documentation at https://lehd.ces.census.gov/data/lodes/LODES8/LODESTechDoc8.0.pdf
    for more information.

    `get_lodes()` is inspired by the lehdr R package (https://github.com/jamgreen/lehdr)
    by Jamaal Green, Dillon Mahmoudi, and Liming Wang.
    ...
    if agg_level != "block":
        if agg_level == "county":
            ...
            raise ValueError(
                "Invalid agg_level; choose one of 'state', 'county', 'tract', "
                "or 'block group'."
            )
    ...
    if return_geometry:
        ...
        if return_lonlat:
            raise ValueError(
                "return_geometry and return_lonlat cannot be used at the same time."
            )
        ...
        if lodes_type == "wac":
            geom = _get_geometry(
                geography=agg_level, state=state, cb=cb, year=year, cache=cache
            )
            ...
            if od_geometry == "home":
                if part == "main":
                    ...
                    aux_states = (
                        lodes_data["h_geocode"].str.slice(stop=2).unique().tolist()
                    )
                    h_geom_list = [
                        _get_geometry(
                            geography=agg_level, state=x, year=year, cb=cb, cache=cache
                        )
                        for x in aux_states
                    ]
        ...
        if lodes_type == "wac":
            ...
            lodes_merged = lodes_data.merge(h_xy, on="h_geocode").merge(
                w_xy, on="w_geocode"
            )
...
def get_xwalk(state, version="LODES8", cache=False):
    """
    Get a Census block-to-parent geography crosswalk file for a given state and a
    given Census year (represented) by a LODES version.
    ...
    version : str
        The LODES version to use.  Version 8 (the default, use "LODES8") is enumerated
        at 2020 Census blocks. Version 7 (use "LODES7") is enumerated at 2010 Census
        blocks.
    cache : bool
        If True, downloads the requested LODES data to a cache directory on your
        computer and reads from that directory if the file exists. Defaults to False,
    ...
    Returns
    -------
    A Pandas DataFrame representing the correspondence between Census blocks and a
    variety of parent geograpies in a given LODES dataset (and in turn a given Census
    year).
    ...
    `get_xwalk()` is inspired by the lehdr R package (https://github.com/jamgreen/lehdr)
    by Jamaal Green, Dillon Mahmoudi, and Liming Wang.
    ...
    url = (
        f"https://lehd.ces.census.gov/data/lodes/{version}/{state}/{state}_xwalk.csv.gz"
    )
