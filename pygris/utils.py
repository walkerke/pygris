...
def erase_water(input, area_threshold=0.75, year=None, cache=False):
    ...
    if len(county_ids) == 0:
        raise ValueError(
            "Your dataset does not appear to be in the United States; "
            "this function is not appropriate for your data."
        )
...
def shift_geometry(input, geoid_column=None, preserve_area=False, position="below"):
    ...
    # Alaska/Hawaii/PR centroids are necessary to put any dataset in the correct
    # location
    ak_crs = 3338
    hi_crs = "ESRI:102007"
    pr_crs = 32161
    ...
    def place_geometry_wilke(geometry, position, centroid, scale=1):
        centroid_x = centroid.x.to_numpy()[0]
        centroid_y = centroid.y.to_numpy()[0]
    ...
    bb = minimal_states.query(
        'GEOID not in ["02", "15", "72"]', engine="python"
    ).total_bounds

    us_lower48 = input_albers.query(
        'state_fips not in ["02", "15", "72"]', engine="python"
    )
    ...
    if pd.concat([us_alaska, us_hawaii, us_puerto_rico]).shape[0] == 0:
        UserWarning(
            "None of your features are in Alaska, Hawaii, or Puerto Rico, so no "
            "geometries will be shifted.\nTransforming your object's "
            "CRS to 'ESRI:102003'"
        )
