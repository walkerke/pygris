...
def _load_tiger(url, cache=False, subset_by=None, protocol="http", timeout=1800):
    ...
    if download_needed:
        ...
        if not download_success:
            raise ValueError(
                "Download failed with both HTTP and FTP; check your internet "
                "connection or the status of the Census Bureau website "
...
def validate_state(state, quiet=False):
    ...
    if len(state) == 2:
        ...
        if state_sub.shape[0] == 0:
            raise ValueError(
                "You have likely entered an invalid state code, please revise."
            )
        else:
...
def validate_county(state, county, quiet=False):
    ...
    # Otherwise, if they pass a name:
        ...
        county_sub = county_table.query(
            "county.str.contains(@county, flags = @re.IGNORECASE, regex = True)",
            engine="python",
        )
        ...
        if len(possible_counties) == 0:
            ...
            cty_code = county_sub.query(
                "county == @possible_counties[0]"
            ).county_code.unique()[0]
            ...
            msg = (
                f"Your string matches {' and '.join(possible_counties)}. "
